...
var DESKLET_ALTERNATE_SIDE_INTERVAL;
...
function init() {
    ...
    return initEnabledDesklets().then(function() {
        ...
        global.settings.connect('changed::' + 'desklet-alternate-side', _onDeskletAlternateSidesChanged);
        global.settings.connect('changed::' + 'desklet-alternate-side-interval', _onDeskletAlternateSidesIntervalChanged);
...
function _onDeskletAlternateSidesChanged() {
    const enable = global.settings.get_boolean('desklet-alternate-side');
    if (!enable) {
        clearInterval(DESKLET_ALTERNATE_SIDE_INTERVAL);
    }
}

function _onDeskletAlternateSidesIntervalChanged() {
    clearInterval(DESKLET_ALTERNATE_SIDE_INTERVAL);
    const minute_interval = global.settings.get_int('desklet-alternate-side-interval');
    const milliseconds = minute_interval * 1000 * 60;
    DESKLET_ALTERNATE_SIDE_INTERVAL = setInterval(moveDesklets, milliseconds);
}
